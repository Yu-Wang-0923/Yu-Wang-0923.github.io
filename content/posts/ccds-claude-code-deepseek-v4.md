...
长对话会消耗大量上下文窗口，导致 AI 遗忘早期内容。键入 `/compact` 会触发对话总结与压缩：

- Claude Code 自动总结之前的对话要点
- 把早期内容替换为精炼摘要
- 释放上下文窗口给后续对话使用

一个实用策略：每当感觉 AI 开始"遗忘"之前的讨论，或者在处理大型代码库中后期感觉上下文不够用时就 `/compact` 一下。压缩后对话可以继续深入而不会因上下文耗尽被截断。
